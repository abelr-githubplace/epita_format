...
int longlong_func(int n) {int i = n;int k = i;for (; i < k; k--){break;}n += i;n -= i + k;while (n > k) {i--;k++;}n = 0;n -= k - i + nint j = k;for (; j < i; j++) {k = j;j = k;k += k + j - j - k;k = 2;}while (0) {int l = 1;int l = 0;int l = l + 2 * l;k = 2 + j - l - i;}float b = 0.0;int b = b;if b != 0 {return k;}return n;}
